...
// An array of all the arrays above -- check to see if function is working
...
const validateCred = arr => {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
        let modifiedValue = arr[i];

        if ((arr.length - 1 - i) % 2 === 0) { // they had 1 here
            modifiedValue *= 2;
        ...
        summedValue += modifiedValue;
    }

    return summedValue % 10 === 0;
};
/* After the fact checker
...
const findInvalidCards = batchArr => {
    ...
    batchArr.forEach(cardArray => { // they had an extra line of code here
        if (!validateCred(cardArray)) {
            invalidArr.push(cardArray);
        }
    });
...
//Identify CC companies with invalid cards
const idInvalidCardCompanies = invalidBatchArr => {
    const companyArr = [];

    invalidBatchArr.forEach(cardArray => { //they had a total diff solution here
        ...
        //this is a nice way to push them without duplicates
        if (!companyArr.includes(companyName)) {
